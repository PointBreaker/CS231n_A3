...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        '''
        note: nn.Embedding() is used to store word embeddings and retrieve them using indices.
        '''
    ...
    def forward(self, features, captions):
        ...
        captions_embed = self.embedding(captions) # (N, T) -> (N, T, W)
        ...
        '''
         note: tril returns the lower triangular part of the matrix (2-D tensor) or 
         batch of matrices input, the other elements of the result tensor out are set to 0.
        '''
        # 3) Apply decoder
        features_embed = self.visual_projection(features) # (N, D) -> (N, W)
        features_embed = features_embed.unsqueeze(1) # (N, W) -> (N, 1, W)
        '''
         note: unsqueeze() is used to add a dimension of size 1 at the specified position.
         '''
        decoder_output = self.transformer(captions_embed, features_embed, tgt_mask=tgt_mask) # (N, T, W)
